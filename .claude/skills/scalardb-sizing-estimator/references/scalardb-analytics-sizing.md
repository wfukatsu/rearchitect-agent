...
> **注意**: GCP Dataproc は ScalarDB Analytics では現時点で正式サポート対象外です。
> GCP環境で分析基盤を構築する場合は、Databricks on GCP の利用を検討してください。
...
| **成長予測** | 1-3年のデータ増加予測 | 将来のSDBU拡張に影響 |
...
#### 費用計算について（AWS Marketplace）

AWS Marketplace版は従量課金（メータリング）方式です。

```
単価: $0.0000232/unit

※ 「メータリング単位（unit）」の詳細定義（vCPU時間、データ量等との対応）は
   AWS Marketplace上では公開されていないため、正確な事前見積もりは困難です。
※ 費用算出には以下のいずれかを推奨します:
   1. Scalar社営業担当に問い合わせて見積もりを取得
   2. PoC環境で実際に稼働させ、実績値から月額費用を推算
※ 別途EMR/Databricksのインフラ費用が必要
```

> **エージェント向け注意**: サイジング見積もり時にAWS Marketplace課金を選択された場合、
> Analytics Serverのライセンス費用は「要問い合わせ」として出力し、
> インフラ費用（EMR/Databricks）のみ算出してください。
