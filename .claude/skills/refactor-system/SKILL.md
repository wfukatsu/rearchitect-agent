...
- フェーズ依存の入力ファイルが不足 → 該当フェーズのスキルを個別に先行実行するよう案内
- 途中フェーズで失敗 → `--resume-from` オプションで失敗フェーズから再開を提案
- 大規模コードベースでタイムアウト → `--domain` オプションでスコープを限定
- サブエージェントの起動失敗 → 個別スキルでの直接実行にフォールバック
